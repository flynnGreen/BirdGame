...
namespace GameDev2D
{
	AnimatedSprite::AnimatedSprite(const std::string& aFilename, AnimatedSpriteCallback* callback) : SpriteAtlas(aFilename),
		m_Callback(callback),
		m_FrameIndex(0),
		m_FrameSpeed(15.0f),
		m_ElapsedTime(0.0),
		...
		if (aEvent->GetEventCode() == UPDATE_EVENT)
		{
			...
			if (m_Pause == false)
			{
				...
				if (m_DoesLoop == true)
				{
					...
					{
						m_Callback->AnimatedSpriteDidLoop(this);
					}
				}
				else
				{
					if (frameIndex == GetFrameCount() - 1)
					{
						...
						if (m_Callback != nullptr)
						{
							m_Callback->AnimatedSpriteIsDone(this);
						}
					}
				}
	...
	void AnimatedSprite::SetCallback(AnimatedSpriteCallback* callback)
	{
		m_Callback = callback;
	}

	AnimatedSpriteCallback* AnimatedSprite::GetCallback()
	{
		return m_Callback;
	}
}
